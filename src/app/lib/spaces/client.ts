...
export async function createSpace(name: string, opts?: { isNameAutoGenerated?: boolean }): Promise<{ created: boolean } | ErrorEnvelope> {
  const res = await fetch('/api/spaces', {
    method: 'POST',
    headers: await authHeaders({ 'Content-Type': 'application/json' }),
    body: JSON.stringify({ name, isNameAutoGenerated: opts?.isNameAutoGenerated === true }),
  });
  if (!res.ok) {
    try { return (await res.json()) as ErrorEnvelope; } catch { return { error: { code: 'HTTP_ERROR', message: String(res.status) } }; }
  }
  ...
  try {
    if (typeof window !== 'undefined' && json && (json.created === true || json.created === false)) {
      window.dispatchEvent(new CustomEvent('spaces:spaceCreated', { detail: { name, isNameAutoGenerated: opts?.isNameAutoGenerated === true } }));
    }
  } catch {}
  return json;
}

export async function getSpaceManifest(spaceName: string): Promise<{ manifest: any } | ErrorEnvelope> {
  const url = `/api/spaces/${encodeURIComponent(spaceName)}/manifest`;
  const res = await fetch(url, { headers: await authHeaders() });
  if (!res.ok) {
    try { return (await res.json()) as ErrorEnvelope; } catch { return { error: { code: 'HTTP_ERROR', message: String(res.status) } }; }
  }
  return (await res.json()) as { manifest: any };
}

export async function renameSpace(currentName: string, newName: string): Promise<{ renamed: boolean; manifest?: any } | ErrorEnvelope> {
  const url = `/api/spaces/${encodeURIComponent(currentName)}`;
  ...
  const json = (await res.json()) as { renamed: boolean; manifest?: any };
  try {
    if (typeof window !== 'undefined' && json?.renamed) {
      window.dispatchEvent(new CustomEvent('spaces:spaceRenamed', { detail: { oldName: currentName, newName, manifest: json.manifest } }));
    }
  } catch {}
  return json;
}
