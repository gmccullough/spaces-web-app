...
export async function createSpace(displayName: string, opts?: { isNameAutoGenerated?: boolean }): Promise<{ created: boolean } | ErrorEnvelope> {
  const name = slugifySpaceName(displayName);
  const res = await fetch('/api/spaces', {
    method: 'POST',
    headers: await authHeaders({ 'Content-Type': 'application/json' }),
    body: JSON.stringify({ name, displayName, isNameAutoGenerated: opts?.isNameAutoGenerated === true }),
  });
  if (!res.ok) {
    try { return (await res.json()) as ErrorEnvelope; } catch { return { error: { code: 'HTTP_ERROR', message: String(res.status) } }; }
  }
  ...
  try {
    if (typeof window !== 'undefined' && json && (json.created === true || json.created === false)) {
      window.dispatchEvent(new CustomEvent('spaces:spaceCreated', { detail: { name, displayName, isNameAutoGenerated: opts?.isNameAutoGenerated === true } }));
    }
  } catch {}
  return json;
}
...
export async function renameSpace(currentName: string, newDisplayName: string): Promise<{ renamed: boolean; manifest?: any } | ErrorEnvelope> {
  const newName = slugifySpaceName(newDisplayName);
  const url = `/api/spaces/${encodeURIComponent(currentName)}`;
  const res = await fetch(url, {
    method: 'PATCH',
    headers: await authHeaders({ 'Content-Type': 'application/json' }),
    body: JSON.stringify({ newName, displayName: newDisplayName }),
  });
  if (!res.ok) {
    try { return (await res.json()) as ErrorEnvelope; } catch { return { error: { code: 'HTTP_ERROR', message: String(res.status) } }; }
  }
  ...
  try {
    if (typeof window !== 'undefined' && json?.renamed) {
      window.dispatchEvent(new CustomEvent('spaces:spaceRenamed', { detail: { oldName: currentName, newName, displayName: newDisplayName, manifest: json.manifest } }));
    }
  } catch {}
  return json;
}
...
export function slugifySpaceName(input: string): string {
  ...
  const slug = base.replace(/[^a-z0-9]+/g, '-').replace(/-{2,}/g, '-').replace(/^-+|-+$/g, '');
  return slug || 'space';
}
