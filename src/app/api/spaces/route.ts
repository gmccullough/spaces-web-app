    ...
    const items = await Promise.all(dirEntries.map(async (i: any) => {
      ...
      try {
        ...
        if (existing?.last_updated_at) {
          ...
          try {
            await writeManifest(user.id, name, {
              version: 1,
              name: existing?.name || name,
              created_at: existing?.created_at || now,
              last_updated_at: lastUpdatedAt,
              is_name_auto_generated: existing?.is_name_auto_generated ?? false,
            });
          } catch {}
        }
        if (existing?.name) {
        ...
        isAutoNamed = existing?.is_name_auto_generated === true;
      } catch {
        // Ignore errors per space
      }
      return { name, displayName, lastUpdatedAt, isNameAutoGenerated: isAutoNamed };
    }));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const isNameAutoGenerated = body?.isNameAutoGenerated === true;
    ...
    const displayName = typeof displayNameRaw === 'string' && displayNameRaw.trim() ? displayNameRaw.trim() : rawName;
