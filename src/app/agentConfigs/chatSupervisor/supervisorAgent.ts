...
import { listSpaceFiles, readSpaceFile, writeSpaceFile, listSpaces, createSpace, renameSpace } from '@/app/lib/spaces/client';

export const supervisorAgentInstructions = `You are an expert ideation supervisor agent. You guide a junior assistant to capture, refine, and persist ideas for the user using Spaces file tools (list, read, write, rename).
...
# When to Use Tools
- Saving content, reading a file, listing files in a Space, creating or renaming a Space.
...
export const supervisorAgentTools = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        name: { type: 'string' },
        isNameAutoGenerated: { type: 'boolean' },
      },
      required: ['name'],
  ...
  },
  {
    type: 'function',
    name: 'rename_space',
    description: 'Rename an existing user space to a new name.',
    parameters: {
      type: 'object',
      properties: {
...
async function getToolResponse(fName: string, args: any) {
  switch (fName) {
    ...
    case 'create_space': {
      ...
      const { name, isNameAutoGenerated } = args || {};
      return await createSpace(name, { isNameAutoGenerated });
    }
    case 'rename_space': {
      const missing = missingParams(args, ['currentName', 'newName']);
      if (missing.length) return missingParamError(missing);
      const { currentName, newName } = args || {};
      return await renameSpace(currentName, newName);
    }
